...
**答：** 我希望用一条完整业务链练习 Java 后端，而不只是实现孤立接口。项目从工单创建逐步扩展到认证、所有权、状态并发、指派和审计，再以同级 Python 服务接入受控 AI 能力，能够验证分层、数据库演进、安全、事务与跨服务边界。

### A2. 项目当前解决了什么问题？【高频】

**答：** 当前解决用户注册登录、三角色授权、工单创建查询分页、顺序状态流转、ADMIN 指派和操作留痕，并以 Redis Lua 实现登录固定窗口限流和创建幂等；P9 通过 Java 调用 FastAPI 提供结构化分析建议、人工审核回复草稿和受控只读 Agent。身份来自 JWT，状态和指派使用条件更新，业务修改与日志同事务；通知、评论和日志查询仍未实现。
...
**答：** AI 表示在可靠工单业务之上的受控能力。当前 Java 仍掌握身份、授权和业务真相，Python 返回结构化建议、人工审核草稿、只读 Agent 结果以及 RAG/MCP 来源，不直接修改工单；默认使用 fake provider，真实 LLM 需要外部凭证。

### A7. 当前最有说服力的项目证据是什么？

**答：** 除了可定位的生产代码和 V1～V6，项目有62个 Surefire 测试类、463项 Java 测试实例，并通过真实 MySQL 和 Redis 验证外键、事务回滚、JWT、限流 Lua、幂等状态机和 AI HTTP 故障边界；Python 另有12项 pytest。但这些不是压力测试，也不等于生产运行证据。

### A8. 如果继续开发，优先做什么？【追问】

**答：** P8 已将安全兜底收紧为 `anyRequest().denyAll()` 并保护 Actuator。下一步应继续完善 AI 外部 provider 的真实凭证 smoke test、操作日志只读查询和“分配给我”，之后再评估可信代理、Token 撤销、Redis 缓存一致性和 MySQL 持久化幂等。
...
### H6. 为什么463项不能都称为端到端测试？【高频】

**答：** 463包含 DTO Validation、Mockito、standalone MVC、Mapper、Service/MySQL、Redis状态机、HTTP Security、AI 客户端故障边界和上下文冒烟等62个测试类。只有部分经过完整 HTTP、安全和数据库/Redis链路，全部称为端到端会夸大证据。
